...
export type SitePart = { k: string; pts: number; d: string; why: string; src: number[] };

export const SITE_PARTS: SitePart[] = [
  { k: "Crawler access", pts: 40, d: "Does robots.txt let the 16 AI crawlers in, or tell them to stay out? Search and user-triggered bots weigh more than training bots, because blocking those is what removes you from answers.", why: "OpenAI and Anthropic each document which of their bots feed search answers and which only train models; the split is theirs, not ours.", src: [16, 17] },
  { k: "JavaScript visibility", pts: 30, d: "AI crawlers read the raw HTML, before scripts run. We compare the raw copy with the finished page and report what share of your words is invisible ink.", why: "Google's own guidance: if a page cannot be indexed with a snippet, it cannot appear in AI features.", src: [4] },
  { k: "llms.txt", pts: 5, d: "A menu card for AI and agent readers. Checked for presence and format.", why: "Deliberately only 5 points. Across 137,000 domains, 97% of llms.txt files were never requested by any bot, and no study has found a citation correlation. Kept because OpenAI and Anthropic recommend it for agent workflows.", src: [12] },
  { k: "Metadata and sitemap", pts: 25, d: "Title, description, canonical, Open Graph, structured data and a working sitemap: the filing system robots rely on.", why: "Indexability basics. Structured data is counted here as a filing aid, not as an AI-citation lever (see what we refuse to score).", src: [4] },
];
...
export const CHECKS: Check[] = [
  ...
  { code: "ad", label: "A visible, recent updated date", pts: 10, looks: "The newest date on the page or in its schema; full marks within 3 months. A schema date newer than the visible date is flagged.", finding: "Content updated within three months averages 6.0 citations vs 3.6. AI-cited content is 25.7% fresher than organic results, except on Google AI Overviews, where freshness is neutral.", src: [2, 9], tag: "evidence" },
