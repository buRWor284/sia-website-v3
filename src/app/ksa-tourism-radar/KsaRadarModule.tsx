...
const INK = "#1a1410";
const YEL = "#f5b81f";
const MARK: Record<KsaLens, MarkShape> = { giga: "disc", events: "square", hosp: "ring", faith: "diamond" };
const CATCOL: Record<KsaLens, string> = { giga: INK, events: INK, hosp: INK, faith: YEL };
const QUAD: Record<KsaLens, number> = {
  giga: -Math.PI / 4,
  events: Math.PI / 4,
  hosp: (3 * Math.PI) / 4,
  faith: (-3 * Math.PI) / 4,
...
const chipClass = (l: KsaLens): string => "ksr-chip" + (l === "faith" ? " c-faith" : "");
...
function Spark({ series, color }: { series: number[]; color: string }) {
  ...
  if (!p) return null;
  return (
    <svg className="ksr-kpi-spark" viewBox="0 0 150 40" preserveAspectRatio="none" aria-hidden="true">
...
function LensMark({ lens }: { lens: KsaLens }) {
  const c = CATCOL[lens];
  const m = MARK[lens];
...
export default function KsaRadarModule({ live }: { live: KsaRadarData }) {
  const [lens, setLens] = useState<LensFilter>("all");
  const [selectedId, setSelectedId] = useState<string>("riyadhair");
  const lensRef = useRef<LensFilter>("all");
  const selRef = useRef<string>("riyadhair");
  useEffect(() => {
  ...
  const liveMap = useMemo(() => {
    const m = new Map<string, KsaLiveTopic>();
    for (const t of live.topics) m.set(t.topic, t);
    return m;
  }, [live.topics]);

  const liveFor = useMemo(() => {
    return (s: KsaSignal): LiveLine | null => {
  ...
  useEffect(() => {
    ...
    const byLens: Record<KsaLens, KsaSignal[]> = { giga: [], events: [], hosp: [], faith: [] };
    for (const s of SIGNALS) byLens[s.lens].push(s);
    const blips: { s: KsaSignal; ang: number; rad: number; ping: number; sx?: number; sy?: number }[] = [];
    (Object.keys(byLens) as KsaLens[]).forEach((cat) => {
    ...
    const drawMark = (x: number, y: number, cat: KsaLens, base: number) => {
      const c = CATCOL[cat];
      const m = MARK[cat];
      if (m === "disc") {
        ctx.beginPath();
    ...
    const draw = () => {
      ...
      ctx.fillText("GIGA-PROJECTS", cx + R * 0.6, cy - R * 0.72);
      ctx.fillText("MEGA-EVENTS", cx + R * 0.62, cy + R * 0.76);
      ctx.fillText("HOSPITALITY", cx - R * 0.64, cy + R * 0.76);
      ctx.fillStyle = "rgba(245,184,31,.95)";
      ctx.fillText("FAITH TRAVEL", cx - R * 0.62, cy - R * 0.72);
      // sweep
      for (let i = 0; i < 36; i++) {
  ...
  const lenses: { l: LensFilter; label: string }[] = [
    { l: "all", label: "§ All" },
    { l: "giga", label: "Giga-projects" },
    { l: "events", label: "Mega-events" },
    { l: "hosp", label: "Hospitality" },
    { l: "faith", label: "Faith travel" },
  ];

  const totalSeries = useMemo(
    () => live.series.giga.map((_, i) => live.series.giga[i] + live.series.events[i] + live.series.hosp[i] + live.series.faith[i]),
    [live.series],
  );

  const visible = (s: KsaSignal): boolean => lens === "all" || s.lens === lens;
  ...
  return (
    <div className="ksr-mod" data-lens={lens}>
      ...
      <div className="ksr-readout" aria-live="polite">
        {lens === "all" ? (
          <>
          ...
          <>
            <span className="ksr-ro-lens">{KSA_LENS_LABEL[lens]}</span> · <b>{SIGNALS.filter((s) => s.lens === lens).length} signals</b>
            {live.hasData ? (
              <>
                {" "}
      ...
      <div className="ksr-stage-grid">
        <div className="ksr-radar-stage">
          <canvas ref={canvasRef} className="ksr-radar-canvas" width={620} height={620} aria-label="Radar of 28 Saudi tourism signals by lens and time horizon. The same data is listed in the signal index below." />
          <div ref={tipRef} className="ksr-radar-tip" />
        </div>

        {/* signal file (detail card) */}
        <aside className="ksr-file" aria-live="polite">
          <div className="ksr-scaps">Signal file · {RING_LABEL[selected.ring - 1]}</div>
          <h3 className="ksr-file-name">{selected.name}</h3>
          <div className="ksr-file-ar" dir="rtl">{selected.ar}</div>
          <div className="ksr-file-badges">
            <span className={chipClass(selected.lens)}>{KSA_LENS_LABEL[selected.lens]}</span>
            <span className="ksr-chip">
              {STATUS_META[selected.status].glyph} {STATUS_META[selected.status].label} · {STATUS_META[selected.status].note}
            </span>
          </div>
          <p className="ksr-file-stat">{selected.stat}</p>
          <p className="ksr-file-sig">
      ...
      <div className="ksr-radar-legend">
        {KSA_LENSES.map((l) => (
          <span className="ksr-lg" key={l}>
            <LensMark lens={l} />
            {KSA_LENS_LABEL[l]}
          </span>
        ))}
      ...
      <div className="ksr-cols">
        {KSA_LENSES.map((l) => (
          <div className="ksr-col" key={l} data-cat={l}>
            <h4 className="ksr-col-head">
              <LensMark lens={l} />
              {KSA_LENS_LABEL[l]}
            </h4>
            <ul>
      ...
      <div className="ksr-mast" style={{ marginTop: 46 }}>
        <span className="ksr-pill">Live wire</span>
        <span className="ksr-scaps">SignalIQ × GDELT BigQuery · Arabic + English press volume</span>
      </div>
      ...
      {live.hasData ? (
        <>
          <div className="ksr-kpis-band">
            <div className="ksr-kpis">
              ...
              <div className="ksr-kpi" data-cat="all">
                <div className="ksr-kpi-label">Heating up now</div>
                <div className="ksr-kpi-val">
                  {live.heating}
                  <span className="u"> / {live.topics.length}</span>
                </div>
                <div className="ksr-kpi-note">{live.risers[0] ? `▲ ${live.risers[0].topic} ${pct(live.risers[0].tr)}` : "topics with rising coverage"}</div>
              </div>
            </div>
          </div>

          <div className="ksr-panels">
        ...
        <div className="ksr-pending">
          <p className="ksr-body">
            ...
            live on the{" "}
            <a href="/earned-media-radar" className="ksr-tlink-inline">
              Earned Media Radar
            </a>
            .
          </p>
          <div className="ksr-skel" />
          <div className="ksr-skel" />
          <p className="ksr-micro">No fabricated trend lines: these stay empty until the first scans land. That is the house rule.</p>
        </div>
      )}
    </div>
  );
