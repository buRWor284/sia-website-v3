...
/* =========================================================================
   PitchClinicDemoTravel — Section 05 live example, travel edition (KSA).
   Same mechanics as PitchClinicDemo on the flagship page: a mock pitch draft
   gets flagged phrase by phrase (the way the room does it in Pitch Clinic and
   Spot the Slop), then cross-fades into the tightened, human-approved version
   that actually goes out. Content localised to a Saudi travel pitch on the
   Middle East travel beat; the numbers are illustrative mock data, as on the
   flagship version. Timer-driven, loops, replayable, inert under
...
const NOTES: ReadonlyArray<Note> = [
  ...
  { text: "revolutionizing global tourism like never before", tag: "Unverified claim" },
  { text: "a game changer for travelers everywhere", tag: "Vague: needs a number" },
...
export default function PitchClinicDemoTravel() {
  ...
  return (
    ...
    >
      ...
      <div className="pcd-lead">
        ...
        <h3 className="pcd-heading">Watching a real travel pitch get tightened, live.</h3>
        <p className="pcd-sub">
          An AI draft opens, gets flagged phrase by phrase the way the room does it in Pitch Clinic and Spot the Slop, then cross-fades into the tightened, human-edited version that actually goes out to a travel journalist.
        </p>
      </div>

      <div className="pcd-card">
        <div className="pcd-header">
          <span className="pcd-title">Pitch Draft &middot; Beat: Middle East Travel</span>
        ...
        <div className={`pcd-body${revised ? " pcd-revised" : ""}`}>
          <div className="pcd-draft">
            <p className="pcd-text">
              Hi [Name], <span className={`pcd-flag${flagged > 0 ? " pcd-flag--on" : ""}`}>I hope this finds you well!</span> Saudi Arabia is{" "}
              <span className={`pcd-flag${flagged > 1 ? " pcd-flag--on" : ""}`}>
                revolutionizing global tourism like never before
              </span>
              , and our new resort is <span className={`pcd-flag${flagged > 2 ? " pcd-flag--on" : ""}`}>a game changer for travelers everywhere</span>.{" "}
          ...
          <div className="pcd-edited">
            <p className="pcd-text">
              Hi [Name], new data: first-time visitor bookings to AlUla rose <span className="pcd-stat">38%</span> this season across the <span className="pcd-stat">40</span> Saudi properties we track, and guest surveys now put AI assistants in the top three trip planning sources. I have the underlying data and three on-record quotes (two hotel GMs, one DMO lead) if useful for the Middle East travel beat. Happy to send the full breakdown.
            </p>
            <div className={`pcd-sent${sent ? " pcd-sent--on" : ""}`}>
              &rarr; Sent to Middle East travel reporter <span className="pcd-check">Sent</span>
            </div>
          </div>
        </div>
      </div>
