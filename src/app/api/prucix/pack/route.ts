 ...
 *
 * Requires ANTHROPIC_API_KEY. Optional: PRUCIX_MODEL, TURNSTILE_SECRET_KEY.
 * POST body: { opportunity, store?, turnstileToken? }
 */
import { NextRequest, NextResponse } from "next/server";
import { rateLimit } from "@/lib/rate-limit";
import { verifyTurnstile } from "@/lib/turnstile";
import { EMAIL_PACKS, FREE_PACKS, PRUCIX_MODEL } from "@/lib/prucix/config";
import {
  PACK_SYSTEM,
  PACK_TOOL,
  assembleSources,
  ...
  buildSignalChart,
  parsePackResult,
} from "@/lib/prucix/assetPrompt";
import { logPack } from "@/lib/prucix/log";
import type { AssetPack, Opportunity, UsageTier } from "@/lib/prucix/types";
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`prucix-pack:${ip}`, { limit, windowMs: MONTH_MS });
  if (!rl.ok) {
  ...
  try {
    ...
    console.error("prucix pack route error:", e);
