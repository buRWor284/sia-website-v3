 ...
 *
 * POST body: { beat, turnstileToken? }   (see src/lib/prucix/types.ts)
 */
import { NextRequest, NextResponse } from "next/server";
import { rateLimit } from "@/lib/rate-limit";
import { verifyTurnstile } from "@/lib/turnstile";
import { EMAIL_SCANS, FREE_SCANS } from "@/lib/prucix/config";
import { scanBeat } from "@/lib/prucix/scan";
import { logScan } from "@/lib/prucix/log";
import type { BeatId, ScanResponse, UsageTier } from "@/lib/prucix/types";
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`prucix-scan:${ip}`, { limit, windowMs: MONTH_MS });
  if (!rl.ok) {
  ...
  try {
    ...
    console.error("prucix scan route error:", e);
