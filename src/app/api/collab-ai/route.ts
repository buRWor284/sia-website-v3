...
[
  {
    ...
    "seoNote": "Why this link matters, in words — e.g. contextual link from a startup financial tool trusted by investors. Do NOT invent a DA or DR number; authority figures are looked up separately.",
...
/**
 * Clamp the model's JSON array of partner suggestions to `limit` rows for the
 ...
 * and no fake scarcity — the results are real, just gated. On any parse failure
...
export async function POST(request: NextRequest) {
  ...
  // (partner-suggestions). The downstream email-writer / campaign-brief calls are
  // per-partner actions and stay on the shared IP brake above — only the search is
  // metered. Keyed by the sia_sub wristband when present (counts follow the user
  // across devices), else hardened IP. Metered → fail-open-with-logging (P2 pattern).
  let previewTier: "anonymous" | "email" = "anonymous";
  let previewRemaining = 0;
  if (type === "partner-suggestions") {
    const quota = await consumeQuota(request, "pciq-preview");
  ...
  try {
    ...
    if (type === "partner-suggestions") {
      const revealLimit = PREVIEW_REVEAL["pciq-preview"][previewTier];
