...
// ── Stage 3: Partners ──────────────────────────────────────────────────────────
function Stage3({ partners, loading, loadingIdx, industry, strategy, biz, selNiches, onToggle, onScore }: {
    ...
    <StageWrapper title="Your partner intelligence." subtitle={`${partners.length} targets found for ${industry} using ${strat.label}. Select the partners you want in your campaign brief.`}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${BD}`, marginBottom: 4, gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={() => {
            const rows = [["Name","URL","Tier","Why","Link Placement","Contact","SEO"],...partners.map(p=>[p.name,p.url,p.tier,p.why,p.linkPage,p.contact,p.seoNote])];
            const csv = rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
            const blob = new Blob([csv],{type:"text/csv"});
            const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
            a.download = `CollabIQ-Partners-${biz||"List"}.csv`; a.click();
          }} style={{ ...ghostBtn(), fontSize: 9, padding: "6px 12px" }}>↓ Download list (CSV)</button>
        </div>
      </div>
      {partners.map((p, i) => {
        ...
        return (
          <div key={i} style={{ borderBottom: `1px solid ${BDS}`, padding: "22px 0" }}>
            ...
            <div className="v2-meta-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px 16px", marginBottom: 12 }}>
              ...
              <div>
                ...
                <a href={`https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(p.contact + " " + p.name)}`} target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: MF, fontSize: 8, color: INFO, marginLeft: 8, textDecoration: "none", letterSpacing: "0.04em" }}>Find on LinkedIn ↗</a>
              </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => onScore(p.name, industry)} style={{ ...ghostBtn(), fontSize: 9, padding: "7px 14px" }}>Score this partner →</button>
...
function Stage4({ state, dispatch, onGated, aiEmail, aiBrief, aiEmailLoading, aiBriefLoading, partners }: {
  ...
  const [tab, setTab] = useState<"score"|"outreach"|"brief">("score");
  const { scPartner, scCat, scores, strategy, biz, domain, desc, industry, customInd, selNiches, audType, geo } = state;
  const ind    = customInd || industry;
  const strat  = V2_STRATEGIES[strategy] || V2_STRATEGIES.discount;
  const scorePct = Math.round((Object.values(scores).reduce((a,b)=>a+b,0)) / (V2_SCORECARD.length * 2) * 100);
  const verdict  = scorePct >= 70 ? "Strong prospect. Prioritise." : scorePct >= 45 ? "Moderate fit. Review weak areas." : scorePct > 0 ? "Low fit. Find a better target." : "";

  const TABS = [{ id:"score",label:"Scorecard"},{id:"outreach",label:"Outreach"},{id:"brief",label:"Brief & Export"}] as const;
      ...
      {tab==="score" && (
        <div>
          <div style={{ marginBottom:24 }}>
            <label style={lbl(TX3)}>Score which partner?</label>
            <select style={inp()} value={scPartner} onChange={e=>{
              const p = partners.find(x=>x.name===e.target.value);
              dispatch({type:"SET",key:"scPartner",val:e.target.value});
              if(p) dispatch({type:"SET",key:"scCat",val:p.tier==="A"?"Priority target":p.tier==="B"?"Strong candidate":"Good to include"});
            }}>
              <option value="">Select a partner to score…</option>
              {partners.map(p=><option key={p.name} value={p.name}>{p.name} (Tier {p.tier}) — {p.url}</option>)}
            </select>
            {scPartner && (
              <div style={{ marginTop:10, padding:"10px 14px", background:"rgba(245,184,31,0.06)", border:`1px solid rgba(245,184,31,0.2)`, display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
                <span style={{ fontFamily:SF, fontSize:15, fontWeight:700, color:TX }}>{scPartner}</span>
                {partners.find(p=>p.name===scPartner) && (
                  <>
                    <span style={{ fontFamily:MF, fontSize:9, color:TX4 }}>{partners.find(p=>p.name===scPartner)!.url}</span>
                    <span style={{ fontFamily:GF, fontSize:12, color:TX3 }}>{partners.find(p=>p.name===scPartner)!.contact}</span>
                  </>
                )}
              </div>
            )}
          ...
          {V2_SCORECARD.map((q,i)=>(
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:16, padding:"14px 0", borderBottom:`1px solid ${BDS}` }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:600, color:TX, lineHeight:1.35, fontFamily:GF }}>{q.q}</div>
                <div style={{ fontSize:11, color:TX3, marginTop:2, fontFamily:GF }}>{q.sub}</div>
              </div>
              <div style={{ display:"flex", gap:4, flexShrink:0 }}>
                {([{l:"Yes",v:2},{l:"Part",v:1},{l:"No",v:0}] as {l:string;v:number}[]).map(b=>{
                  const sel=scores[i]===b.v&&scores[i]!==undefined;
                  const bg=sel?(b.v===2?SUCC:b.v===1?AMB2:ERR):"transparent";
                  return (
                    <button key={b.v} onClick={()=>dispatch({type:"SET_SCORE",idx:i,val:b.v})}
                      style={{ padding:"7px 12px", border:`1px solid ${sel?bg:BD}`, background:bg,
                        fontFamily:MF, fontSize:9, fontWeight:700, color:sel?"#fff":TX3, cursor:"pointer", borderRadius:0 }}>
                      {b.l}
                    </button>
                  );
                })}
          ...
          <div style={{ background:BG2, border:`1px solid ${BD}`, padding:22, marginTop:20, display:"flex", alignItems:"center", gap:24, flexWrap:"wrap" }}>
            <div style={{ textAlign:"center" }}>
              <span style={{ fontFamily:SF, fontSize:48, fontWeight:700, color:ACC, lineHeight:1, display:"block" }}>{scorePct}%</span>
              <span style={{ fontFamily:MF, fontSize:8, fontWeight:700, color:TX4, letterSpacing:"0.12em", textTransform:"uppercase", marginTop:4, display:"block" }}>Fit Score</span>
            </div>
            <div style={{ flex:1, minWidth:120 }}>
              <div style={{ height:3, background:BDS, marginBottom:10 }}>
                <div style={{ height:"100%", background:ACC, width:`${scorePct}%`, transition:"width 0.4s" }} />
              </div>
              <div style={{ fontSize:12, color:scorePct>=70?SUCC:scorePct>=45?AMB2:scorePct>0?ERR:TX4, fontFamily:GF }}>
                {verdict||"Answer the questions above."}
              </div>
            </div>
          </div>
        </div>
...
export function CollabIQ() {
  ...
  function handleGated(action: string) {
    // Brief generation is always free — only PDF download and copy require email
    ...
    <>
      ...
      <div className="v2-collabiq" style={{ background:t.BG1, color:t.TX, minHeight:"100vh" }}>
        ...
        <div key={`stage-${step}`}>
          ...
          {step===3 && <Stage3 partners={partners} loading={loading} loadingIdx={loadingIdx} industry={ind} strategy={state.strategy} biz={state.biz} selNiches={state.selNiches} onToggle={n=>dispatch({type:"TOGGLE_NICHE",val:n})} onScore={(n,c)=>dispatch({type:"SCORE_PARTNER",name:n,cat:c})} />}
