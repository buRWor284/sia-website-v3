...
function Stage3({ partners, loading, loadingIdx, industry, strategy, biz, selNiches, onToggle, onScore }: {
  ...
  return (
    <StageWrapper title="Your partner intelligence." subtitle={`${partners.length} targets found for ${industry} using ${strat.label}. Select the partners you want in your campaign brief.`}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${BD}`, marginBottom: 4 }}>
        ...
        return (
          <div key={i} style={{ borderBottom: `1px solid ${BDS}`, padding: "22px 0" }}>
            ...
            <div className="v2-meta-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px 16px", marginBottom: 12 }}>
              ...
              <div><span style={{ ...lbl(TX4), fontSize: 8, marginBottom: 4 }}>Contact</span><span style={{ fontSize: 12, color: TX2, fontFamily: GF }}>{p.contact}</span></div>
...
// ── Stage 4: Toolkit ───────────────────────────────────────────────────────────
function Stage4({ state, dispatch, onGated, aiEmail, aiBrief, aiEmailLoading, aiBriefLoading }: {
      ...
      {tab==="score" && (
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:24 }}>
            <div><label style={lbl(TX3)}>Company</label><input style={inp()} placeholder="e.g. Loom" value={scPartner} onChange={e=>dispatch({type:"SET",key:"scPartner",val:e.target.value})} /></div>
            <div><label style={lbl(TX3)}>Category</label><input style={inp()} placeholder="e.g. Video Platform" value={scCat} onChange={e=>dispatch({type:"SET",key:"scCat",val:e.target.value})} /></div>
          </div>
      ...
      {tab==="brief" && (
        <div>
          ...
          <button onClick={()=>onGated("brief")} disabled={aiBriefLoading||!biz}
            style={{ ...primaryBtn(), opacity:(!biz||aiBriefLoading)?0.4:1, marginBottom:16 }}>
      ...
      <div style={{ background:BG1, border:`1px solid ${BD}`, maxWidth:420, width:"100%", padding:36 }}>
        {success ? (
          ...
          <form onSubmit={handleSubmit}>
            <span style={{ ...lbl(TX4), marginBottom:14 }}>Unlock downloads</span>
            ...
            <p style={{ fontFamily:GF, fontSize:14, color:TX3, marginBottom:24, lineHeight:1.6 }}>
              Subscribe to Syed Irfan Ajmal&rsquo;s newsletter to download your results. Real case studies, zero filler — one or two emails a month.
            </p>
    ...
    <>
      ...
      <div className="v2-collabiq" style={{ background:t.BG1, color:t.TX, minHeight:"100vh" }}>
        ...
        <div key={`stage-${step}`}>
          ...
          {step===4 && <Stage4 state={state} dispatch={dispatch} onGated={handleGated} aiEmail={aiEmail} aiBrief={aiBrief} aiEmailLoading={aiEmailLoading} aiBriefLoading={aiBriefLoading} />}
        </div>
