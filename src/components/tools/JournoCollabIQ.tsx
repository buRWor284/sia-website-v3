...
function reducer(state: CollabState, action: Action): CollabState {
  switch (action.type) {
    case "SET": {
      const next = { ...state, [action.key]: action.val };
      // Switching the selected journalist invalidates the fit scorecard (it's
      // per-journalist) — clear it so a new pick doesn't inherit old answers.
      if (action.key === "scPartner") next.scores = {};
      return next;
    }
    ...
    case "CLEAR_STAGE1": return { ...state, biz: "", domain: "", desc: "", industry: "", customInd: "", selNiches: [] };
    case "CLEAR_STAGE2": return { ...state, audDesc: "", audType: "", geo: "" };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 24 }}>
        <button onClick={() => dispatch({ type: "CLEAR_STAGE1" })} style={{ ...ghostBtn(), fontSize: 9, padding: "8px 14px" }}>Clear fields</button>
      </div>
    </StageWrapper>
  );
}
...
function Stage2({ state, dispatch }: { state: CollabState; dispatch: React.Dispatch<Action> }) {
  ...
  return (
    <StageWrapper title="What are you offering the journalist?" subtitle="Each angle lands differently. Pick the offer that fits your story, then add the story itself and your target outlet tier.">
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 24 }}>
        <button onClick={() => dispatch({ type: "CLEAR_STAGE2" })} style={{ ...ghostBtn(), fontSize: 9, padding: "8px 14px" }}>Clear story</button>
      </div>
    </StageWrapper>
  );
}
    ...
    <div aria-hidden style={{ borderBottom: `1px solid ${BDS}`, padding: "22px 0" }}>
      ...
      <div style={{ background: BG2, borderLeft: `3px solid ${BD}`, padding: "12px 14px", marginBottom: 14 }}>
        <p style={{ fontSize: 13, color: TX2, lineHeight: 1.65, margin: 0, fontFamily: GF }}>
          This match is ready. Add your email to reveal why it fits, the recent coverage, and the contact.
        </p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px 16px" }}>
        {["Recent coverage", "Contact", "Authority"].map(l => (
          <div key={l}>
...
function Stage3({ partners, hiddenCount, onUnlock, loading, loadingIdx, industry, strategy, biz, selNiches, onToggle, onScore, onGatedCsv, error, onRetry }: {
  ...
  return (
    <StageWrapper title="Your journalist shortlist." subtitle={`${partners.length + hiddenCount} journalists found for ${industry} using ${strat.label}. Select the ones you want in your media brief.`}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${BD}`, marginBottom: 4, gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={onRetry} disabled={loading} style={{ ...ghostBtn(), fontSize: 9, padding: "6px 12px", opacity: loading ? 0.5 : 1, cursor: loading ? "wait" : "pointer" }}>↻ New search</button>
      ...
      {hiddenCount > 0 && (
        <div style={{ position: "relative", marginTop: 4 }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <UnlockOverlay hidden={hiddenCount} kind="journalists" onUnlock={onUnlock} />
          </div>
        </div>
      )}
...
export function JournoCollabIQ({ toolHeaderHeight = 0 }: { toolHeaderHeight?: number }) {
  ...
  // P3: how many journalists the server withheld from an anonymous preview search
  ...
  function goNext() {
    // Only fire a fresh AI search when there are no results yet. Coming back to the
    // Journalists step after navigating away keeps the existing matches — a rescan
    // costs an Opus call + a quota unit, so it's a deliberate "New search" action.
    if(step===2){dispatch({type:"GO",step:3}); if(partners.length===0) generatePartners();}
